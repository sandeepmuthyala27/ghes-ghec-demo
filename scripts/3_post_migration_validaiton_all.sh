...
validate_migration() {
  ...
  [[ ${#missing_in_github[@]} -gt 0 ]] && write_log "⚠️  Missing in GitHub: ${missing_in_github[*]}" || write_log "✅ No branches missing in GitHub"

  [[ ${#missing_in_ghes[@]} -gt 0 ]] && write_log "⚠️  Extra branches in GitHub: ${missing_in_ghes[*]}" || write_log "✅ No extra branches found in GitHub"
  ...
  for branch in "${gh_array[@]}"; do
    ...
    IFS="|" read -r gh_count gh_sha <<< "$(get_commit_count_and_latest github "$github_org" "$github_repo" "$branch")"
    IFS="|" read -r ghes_count ghes_sha <<< "$(get_commit_count_and_latest ghes "$ghes_org" "$ghes_repo" "$branch")"

    [[ "$gh_count" == "$ghes_count" ]] \
      && write_log "✅ Branch: ${branch} | Commit Count MATCHED" \
      || write_log "❌ Branch: ${branch} | Commit Count NOT MATCHED | GHES=${ghes_count} GitHub=${gh_count}"

    [[ "$ghes_sha" == "$gh_sha" && -n "$ghes_sha" ]] \
      && write_log "✅ Branch: ${branch} | Latest SHA MATCHED" \
      || write_log "❌ Branch: ${branch} | Latest SHA NOT MATCHED"
  done
...
validate_from_csv() {
  ...
  [[ ! -f "$csv" ]] && { write_log "❌ CSV file not found: $csv"; exit 1; }
